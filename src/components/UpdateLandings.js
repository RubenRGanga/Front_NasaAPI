...
const apiEndpoint = 'https://ruben-proyecto-nasa-backend.onrender.com/api/astronomy/landings/edit/';

const UpdateLanding = ({_id}) => {
  console.log(apiEndpoint + _id);
  ...
  useEffect(() => {
    async function getPost() {
      const { data } = await axios.get(apiEndpoint + _id);

      setName(data.name);
      setRecclass(data.recclass);
      setYear(data.year);
      setMass(data.mass);
      setReclat(data.reclat);
      setReclong(data.reclong);
  ...
  const handleSubmit = async (e) => {
    ...
    const { data } = await axios.put(apiEndpoint + _id, updateLanding);

      console.log(data);


    alert("Landing data updated correctly");
    navigate('/landings/');
